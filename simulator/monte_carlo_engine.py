...
class GameSimulator:
    ...
    def simulate_game(
        ...
        logger.info("Starting %d simulations (home=%d batters, away=%d batters)", n, len(home_lineup), len(away_lineup))
        ...
        for sim_idx in range(n):
            self._run_single_game(
                ...
                b_hits=b_hits, b_tb=b_tb, b_bb=b_bb, b_k=b_k, b_rbi=b_rbi, b_runs=b_runs,
                p_k=p_k, p_bb=p_bb, p_ha=p_ha, p_ip=p_ip, p_pc=p_pc,
            )
        ...
        return SimulationResult(
            ...
            batter_hits=b_hits, batter_total_bases=b_tb, batter_walks=b_bb,
            batter_strikeouts=b_k, batter_rbis=b_rbi, batter_runs=b_runs,
            pitcher_strikeouts=p_k, pitcher_walks=p_bb, pitcher_hits_allowed=p_ha,
            pitcher_innings=p_ip, pitcher_pitches=p_pc,
        )

    def _run_single_game(
        self, sim_idx, home_lineup, away_lineup, home_pitcher_probs, away_pitcher_probs,
        home_pitcher_id, away_pitcher_id, cfg, pa_resolver, home_scores, away_scores,
        b_hits, b_tb, b_bb, b_k, b_rbi, b_runs, p_k, p_bb, p_ha, p_ip, p_pc,
    ) -> None:
        state = GameState()
        home_pitcher_outs = 0
        away_pitcher_outs = 0
        ...
        for inning in range(1, max_innings + 1):
            for half in ("top", "bottom"):
                ...
                while outs < 3:
                    ...
                    pa_result = pa_resolver.resolve(pitcher_id, batter_id, probs, fatigue_factor=fatigue)
                    pitcher_bf_ref += 1

                    pitch_est = {"K": 5, "BB": 6, "HBP": 3, "1B": 4, "2B": 4, "3B": 4, "HR": 4, "out": 3}.get(pa_result.outcome, 4)
                    ...
                    if pa_result.outcome == "K" or pa_result.outcome == "out":
                        ...
                        runners, rbis = _advance_runners(runners, pa_result.outcome, score_side, score)
                        b_rbi[batter_id][sim_idx] += rbis

                        if pa_result.outcome == "HR":
                            b_runs[batter_id][sim_idx] += 1
    ...
    def summarise(self, result: SimulationResult) -> SimulationSummary:
        ...
        batter_stat_maps = {
            "hits": result.batter_hits, "total_bases": result.batter_total_bases,
            "walks": result.batter_walks, "strikeouts": result.batter_strikeouts,
            "rbis": result.batter_rbis, "runs": result.batter_runs,
        }
        pitcher_stat_maps = {
            "strikeouts": result.pitcher_strikeouts, "walks": result.pitcher_walks,
            "hits_allowed": result.pitcher_hits_allowed, "innings": result.pitcher_innings,
        ...
        return SimulationSummary(
            ...
            batter_stats=batter_stats, pitcher_stats=pitcher_stats, raw=result,
        )
...
def _build_demo_game(n_batters: int = 9):
    ...
    default_probs: dict[str, float] = {
        ...
        "1B": 0.155, "2B": 0.050, "3B": 0.005, "HR": 0.035, "out": 0.435,
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run a demo Monte Carlo MLB game simulation.")
...
OUT_INDICES: set[int] = {K_IDX, FLYOUT_IDX, GROUNDOUT_IDX, 9, 10}
...
def build_batter_probs(
    k_rate: float = 0.22, bb_rate: float = 0.08, hbp_rate: float = 0.01,
    single_rate: float = 0.15, double_rate: float = 0.045, triple_rate: float = 0.004,
    hr_rate: float = 0.03, flyout_rate: float | None = None,
    groundout_rate: float | None = None, lineout_rate: float | None = None,
    popup_rate: float | None = None,
) -> np.ndarray:
    explicit = np.array([k_rate, bb_rate, hbp_rate, single_rate, double_rate, triple_rate, hr_rate], dtype=np.float64)
    ...
    if all_provided:
        out_arr = np.array([flyout_rate, groundout_rate, lineout_rate, popup_rate], dtype=np.float64)
    else:
...
class BatterProfile:
    def __init__(self, mlbam_id: int, name: str, lineup_position: int, probs: np.ndarray | None = None) -> None:
        ...
        if probs.shape != (N_OUTCOMES,):
            raise ValueError(f"BatterProfile probs must have shape ({N_OUTCOMES},), got {probs.shape}")
        if probs.sum() == 0.0:
            self.probs = MLB_AVG_PROBS.copy()
        else:
...
class PitcherProfile:
    def __init__(self, mlbam_id: int, name: str, throws: str = "R",
                 k_rate_modifier: float = 1.0, contact_quality_modifier: float = 1.0,
                 pitch_count_mean: float = 92.0, pitch_count_std: float = 12.0,
                 recent_pitch_counts: list[float] | None = None) -> None:
        self.mlbam_id = mlbam_id
        self.name = name
        self.throws = throws
...
class BullpenProfile:
    def __init__(self, k_rate_modifier: float = 1.0, contact_quality_modifier: float = 1.0,
...
class GameMatchup:
    def __init__(self, pitcher: PitcherProfile, lineup: list[BatterProfile],
                 bullpen: BullpenProfile, park_factor: float = 1.0,
                 weather_factor: float = 1.0, umpire_k_factor: float = 1.0) -> None:
        if len(lineup) != 9:
            raise ValueError(f"GameMatchup lineup must contain exactly 9 batters; got {len(lineup)}.")
...
_STAT_ALIASES: dict[str, str] = {
    "K": "strikeouts", "H": "hits", "TB": "total_bases", "HR": "home_runs",
    "BB": "walks", "R": "runs", "RBI": "rbis", "PA": "plate_appearances",
    "strikeouts": "strikeouts", "hits": "hits", "total_bases": "total_bases",
    "home_runs": "home_runs", "walks": "walks", "runs": "runs",
    "rbis": "rbis", "plate_appearances": "plate_appearances",
}


class PlayerSimResults:
    def __init__(self, mlbam_id: int, name: str, n_sims: int,
                 strikeouts, hits, total_bases, home_runs, walks, runs, rbis, plate_appearances) -> None:
        self.mlbam_id = mlbam_id
        self.name = name
        self.n_sims = n_sims
    ...
    def to_dict(self) -> dict:
        ...
        for short, full in [("K", "strikeouts"), ("H", "hits"), ("TB", "total_bases"),
                            ("HR", "home_runs"), ("BB", "walks"), ("R", "runs"),
                            ("RBI", "rbis"), ("PA", "plate_appearances")]:
            arr = self._arrays[full]
            counts, edges = np.histogram(arr, bins=max(1, int(arr.max()) + 2 if arr.max() > 0 else 2),
                                          range=(0, max(1, int(arr.max()) + 1)))
            stats_dict[short] = {
                "mean": float(np.mean(arr)), "std": float(np.std(arr)),
                "median": float(np.median(arr)), "p10": float(np.percentile(arr, 10)),
                "p90": float(np.percentile(arr, 90)),
                "histogram": counts.tolist(), "bin_edges": edges.tolist(),
            }
        return {"mlbam_id": self.mlbam_id, "name": self.name, "n_sims": self.n_sims, "stats": stats_dict}


class GameSimResults:
    def __init__(self, n_sims: int, player_results: dict, team_runs: np.ndarray, pitcher_pitch_counts: np.ndarray) -> None:
        self.n_sims = n_sims
    ...
    def to_dict(self) -> dict:
        return {
            "n_sims": self.n_sims,
            "players": {str(mid): pr.to_dict() for mid, pr in self.player_results.items()},
            "team_runs": {"mean": float(np.mean(self.team_runs)), "std": float(np.std(self.team_runs)), "values": self.team_runs.tolist()},
            "pitcher_pitch_counts": {"mean": float(np.mean(self.pitcher_pitch_counts)), "std": float(np.std(self.pitcher_pitch_counts)), "values": self.pitcher_pitch_counts.tolist()},
        }


def build_pitcher_profile_from_stats(mlbam_id, name, career_k9=8.5, throws="R", recent_pitch_counts=None):
    k_rate_modifier = float(np.clip(career_k9 / 8.5, 0.5, 1.6))
    contact_quality_modifier = float(np.clip(0.8 + (career_k9 - 8.5) * 0.02, 0.7, 1.3))
    return PitcherProfile(mlbam_id=mlbam_id, name=name, throws=throws,
                          k_rate_modifier=k_rate_modifier, contact_quality_modifier=contact_quality_modifier,
                          recent_pitch_counts=recent_pitch_counts)


def build_bullpen_profile(era=4.0, k9=8.5):
    k_rate_modifier = float(np.clip(k9 / 8.5, 0.5, 1.6))
    contact_quality_modifier = float(np.clip(4.0 / max(era, 0.5), 0.6, 1.5))
    return BullpenProfile(k_rate_modifier=k_rate_modifier, contact_quality_modifier=contact_quality_modifier)


def _apply_pitcher_modifiers(base_probs, pitcher, is_bullpen, bullpen, park_factor, weather_factor, umpire_k_factor):
    probs = base_probs.copy()
    k_mod = bullpen.k_rate_modifier if is_bullpen else pitcher.k_rate_modifier
    cq_mod = bullpen.contact_quality_modifier if is_bullpen else pitcher.contact_quality_modifier
...
_PITCHES_PER_PA: np.ndarray = np.array([5, 6, 3, 4, 4, 4, 4, 3, 3, 3, 3], dtype=np.float64)


def _simulate_game_single(matchup, rng, n_innings=9):
    n_batters = 9
    ...
    pc_threshold = max(50.0, pc_threshold)
    pitcher_pitches = 0.0
    total_pitches = 0.0
    pitcher_ks = 0
    order_idx = 0
    ...
    for inning in range(n_innings):
        ...
        while outs < 3:
            ...
            mod_probs = _apply_pitcher_modifiers(
                batter.probs, pitcher=pitcher, is_bullpen=using_bullpen,
                bullpen=matchup.bullpen, park_factor=matchup.park_factor,
                weather_factor=matchup.weather_factor, umpire_k_factor=matchup.umpire_k_factor,
            )
            draw = rng.random()
            ...
            if outcome_idx == K_IDX:
                ...
                if runs > 0:
                    _credit_runs(runner_slots, bases, new_bases, b_r, None)
                team_runs += runs
                b_rbi[slot] += rbis
    ...
    stats = {"k": b_k, "h": b_h, "tb": b_tb, "hr": b_hr, "bb": b_bb, "r": b_r, "rbi": b_rbi, "pa": b_pa}
    return stats, float(team_runs), float(pitcher_pitches), pitcher_ks


def _push_runner_walk(runner_slots, batter_slot):
...
def _credit_runs(runner_slots, old_bases, new_bases, b_r, batter_slot):
...
def _credit_runs_advanced(runner_slots, old_bases, new_bases, b_r, extra_runs):
    for base_idx in range(3):
        if old_bases[base_idx] and not new_bases[base_idx]:
            slot = runner_slots[base_idx]
            if slot is not None:
...
def simulate_game(matchup, n_sims=3000, seed=None):
    ...
    for i in range(n_sims):
        ...
        b_k[:, i] = stats["k"]; b_h[:, i] = stats["h"]; b_tb[:, i] = stats["tb"]
        b_hr[:, i] = stats["hr"]; b_bb[:, i] = stats["bb"]; b_r[:, i] = stats["r"]
        b_rbi[:, i] = stats["rbi"]; b_pa[:, i] = stats["pa"]
        team_runs_arr[i] = tr; pitcher_pc_arr[i] = pc
    player_results = {}
    for slot, batter in enumerate(lineup):
        player_results[batter.mlbam_id] = PlayerSimResults(
            mlbam_id=batter.mlbam_id, name=batter.name, n_sims=n_sims,
            strikeouts=b_k[slot], hits=b_h[slot], total_bases=b_tb[slot],
            home_runs=b_hr[slot], walks=b_bb[slot], runs=b_r[slot],
            rbis=b_rbi[slot], plate_appearances=b_pa[slot],
        )
    return GameSimResults(n_sims=n_sims, player_results=player_results,
                          team_runs=team_runs_arr, pitcher_pitch_counts=pitcher_pc_arr)


def simulate_game_with_pitcher_ks(matchup, n_sims=3000, seed=None):
    rng = np.random.default_rng(seed)
    n_batters = 9
    lineup = matchup.lineup
    b_k = np.zeros((n_batters, n_sims), dtype=np.int32)
    ...
    for i in range(n_sims):
        ...
        b_hr[:, i] = stats["hr"]; b_bb[:, i] = stats["bb"]; b_r[:, i] = stats["r"]
        b_rbi[:, i] = stats["rbi"]; b_pa[:, i] = stats["pa"]
        team_runs_arr[i] = tr; pitcher_pc_arr[i] = pc; pitcher_ks_arr[i] = pks
    player_results = {}
    for slot, batter in enumerate(lineup):
        player_results[batter.mlbam_id] = PlayerSimResults(
            mlbam_id=batter.mlbam_id, name=batter.name, n_sims=n_sims,
    ...
    game_results = GameSimResults(n_sims=n_sims, player_results=player_results,
                                   team_runs=team_runs_arr, pitcher_pitch_counts=pitcher_pc_arr)
